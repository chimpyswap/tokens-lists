# ChimpySwap Token Lists

This repo contains main ChimpySwap token list and tools to validate it.
...
- Add `checksum:newlistname`, `generate:newlistname`, `makelist:newlistname` command to `package.json` analogous to ChimpySwap default and extended list scripts.
- Modify `checksum.ts`, `buildList.ts`, `ci-check.ts`, and `default.test.ts` to handle new list

## How to add new tokens to ChimpySwap (extended) token list
...
```shell script
# Fetch the Top100 Tokens on ChimpySwap v2, and update list.
...
Netlify simply takes the json files under `lists` directory and hosts them on `tokens.chimpyswap.com/list-name.json`
